HOME_DIR=$(pwd)
export CONFIG_FILES="${1:-$HOME_DIR/bob-testnet/artifacts/agent-config-2024-03-07-23-25-38.json}"
echo "Running relayer with config files: $CONFIG_FILES"

RELAYER_KEY=${2:-$(cat $HOME_DIR/bob-testnet/.keys/relayerkey)}
...
CONFIG_FILES=$CONFIG_FILES $BINARY_FILE \
    --db ./hyperlane_db_bobtestnet_relayer \
    --relayChains bobtestnet,sepolia \
