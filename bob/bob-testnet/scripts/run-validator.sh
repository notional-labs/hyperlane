HOME_DIR=$(pwd)
CONFIG_FILES="${1:-$HOME_DIR/bob-testnet/artifacts/agent-config-2024-03-07-23-25-38.json}"
echo "Using config files: $CONFIG_FILES"

# Pick an informative name specific to the chain you're validating
mkdir -p $HOME_DIR/tmp
VALIDATOR_SIGNATURES_DIR=$HOME_DIR/tmp/hyperlane-validator-signatures-bobtestnet
...
VALIDATOR_KEY=${2:-$(cat $HOME_DIR/bob-testnet/.keys/valkey)}
...
CONFIG_FILES=$CONFIG_FILES $BINARY_FILE \
    --db ./hyperlane_db_bobtestnet_validator \
    --originChainName bobtestnet \
